...
void listadoOrdenadoAutosPtoD(vecAuto,int);
...
int main(){
	...
	listadoOrdenadoAutosPtoD(autos,cantAutos);
...
void listadoOrdenadoAutosPtoD(vecAuto a,int b){
	cout<<"Autos no alquilados: "<<endl;
		
		for(int i=0;i<b;i++){
		
		if(a[i].estado==false){
			cout<<"Patente: "<<a[i].patente<<endl;
		}
	}
...
void devolucion(vecAuto a, int b){
	...
	while(strcmp(patente.c_str(),"0")!=0){
		...
		cout<<"Patente de auto a devolver:(ingrese 0 para salir) "<<endl;
		cin>>patente;
		if(strcmp(patente.c_str(),"0")!=0){
			...
			a[posicion].cantKm=a[posicion].cantKm+a[posicion].kmRecorridos;
		}else{
			break;
		}
	}
...
void ingresarAutos(vecAuto a,int &cantAutos){
	...
	while (!ar.eof()){
		...
		ultimo=cadena.find(";",primero);
		if ((strcmp(cadena.substr(primero,ultimo-primero).c_str(),"D")==0)){
			a[i].estado=false;
		}else {
			a[i].estado=true;
		}
